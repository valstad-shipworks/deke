...
fn two_stage_solve<const N: usize>(
    ...
    // Pre-condition the stage-1 solution into stage 2's feasible region before
    // warm-starting. Stage 1 has no TCP constraints, so its `sd[k]` may sit above the
    // per-sample TCP-v cap `tcp.v_max / ‖pp[k]‖`. Handing that directly to stage 2
    // forces the IPM to drag every variable down by the same factor *while* it figures
    // out the TCP a/j feasible region — exactly the multi-objective tug-of-war that
    // sends it into feasibility restoration on rest-to-rest p2p moves where joint
    // jerk is also bang-bang at sample 0.
    //
    // Uniform time-rescaling `t → t / scale`: sd → scale·sd, sdd → scale²·sdd,
    // sddd → scale³·sddd, dt → dt/scale. This preserves the integrator and ds equalities
    // exactly (each term picks up `scale^k` factors that cancel). We pick `scale` so the
    // rescaled sd stays under the per-sample TCP-v cap with a small margin; if no scaling
    // is needed (stage 1 already under the cap everywhere), `scale = 1.0` and the warm
    // start is unchanged.
    let stage1 = rescale_warm_start::<N>(stage1, deriv_with_tcp, constraints);

    // Stage 2: TCP-enabled with the (rescaled) stage-1 solution as warm start.
    let stage2 = build_and_solve_warm::<N>(deriv_with_tcp, constraints, start, end, &stage1)?;
    Ok(stage2)
}

/// Uniformly slow down a stage-1 solution so its `sd[k]` profile fits under the TCP
/// velocity cap that stage 2 will enforce. Returns the stage-1 solution unchanged if no
/// TCP v cap is active or stage 1 is already feasible everywhere.
///
/// The factor is `min over k of (tcp.v_max / (‖pp[k]‖ · sd[k]))` with a 5% safety
/// margin and clamped to [0.05, 1.0]. The 5% margin gives the IPM interior room
/// against the TCP v bound; the 0.05 floor prevents absurd downscalings when one
/// sample has a tiny `‖pp‖` (and thus a huge cap that ws.sd doesn't actually hit).
fn rescale_warm_start<const N: usize>(
    mut sol: crate::nlp::Solution,
    deriv: &PathDerivatives<N>,
    constraints: &Topp3Tcp6Constraints<N>,
) -> crate::nlp::Solution {
    let tcp = match constraints.tcp {
        Some(t) if t.v_max.is_finite() && t.v_max > 0.0 && deriv.has_tcp() => t,
        _ => return sol,
    };
    let m = sol.sd.len();
    if m == 0 {
    ...
    // Same cutoff as nlp.rs uses when deciding whether to add the TCP v row: skip
    // samples where ‖pp‖ collapses (wrist-only rotation etc.), since their "cap" is
    // astronomical and never binding.
    let pp_cutoff_sq = 1e-6 * max_pp_sq;
    ...
    // 5% safety margin under the TCP v cap so stage 2's IPM has interior room.
    scale = (scale * 0.95).clamp(0.05, 1.0);
    if scale >= 0.999 {
        return sol;
    }
    ...
    for k in 0..sol.sddd.len() {
        sol.sddd[k] *= s3;
    }
    for k in 0..sol.dt.len() {
        sol.dt[k] = std::time::Duration::from_secs_f64(sol.dt[k].as_secs_f64() * inv_scale);
    }
    sol
}
