...
fn main() {
    ...
    println!("=== deke RRTC planner ===");
    println!("robot: r2000ic270f ({DOF} DOF)");

    let rec = rerun::RecordingStreamBuilder::new("deke-viz-rrtc")
        .spawn()
    ...
    let settings = deke_rrt::RrtcSettings::new(SRobotQ(JOINT_LOWER), SRobotQ(JOINT_UPPER));
    let planner = rrtc(settings);
