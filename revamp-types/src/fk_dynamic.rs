use glam::Affine3A;

use crate::{DHChain, DHJoint, HPChain, HPJoint, URDFChain, URDFJoint};
use crate::{FKChain, RevampError, RevampResult, SRobotQ};

macro_rules! dynamic_fk {
    ($name:ident, $chain:ident, $joint:ident) => {
        #[derive(Debug, Clone)]
        pub enum $name {
            J1($chain<1>),
            J2($chain<2>),
            J3($chain<3>),
            J4($chain<4>),
            J5($chain<5>),
            J6($chain<6>),
            J7($chain<7>),
            J8($chain<8>),
        }

        impl $name {
            pub fn try_new(joints: Vec<$joint>) -> RevampResult<Self> {
                Ok(match joints.len() {
                    1 => Self::J1($chain::new(joints.try_into().ok().unwrap())),
                    2 => Self::J2($chain::new(joints.try_into().ok().unwrap())),
                    3 => Self::J3($chain::new(joints.try_into().ok().unwrap())),
                    4 => Self::J4($chain::new(joints.try_into().ok().unwrap())),
                    5 => Self::J5($chain::new(joints.try_into().ok().unwrap())),
                    6 => Self::J6($chain::new(joints.try_into().ok().unwrap())),
                    7 => Self::J7($chain::new(joints.try_into().ok().unwrap())),
                    8 => Self::J8($chain::new(joints.try_into().ok().unwrap())),
                    n => return Err(RevampError::ShapeMismatch { expected: 8, found: n }),
                })
            }

            pub fn dof(&self) -> usize {
                match self {
                    Self::J1(_) => 1,
                    Self::J2(_) => 2,
                    Self::J3(_) => 3,
                    Self::J4(_) => 4,
                    Self::J5(_) => 5,
                    Self::J6(_) => 6,
                    Self::J7(_) => 7,
                    Self::J8(_) => 8,
                }
            }

            pub fn fk_dyn(&self, q: &[f32]) -> RevampResult<Vec<Affine3A>> {
                dynamic_fk!(@dispatch_fk self, q,
                    J1 1, J2 2, J3 3, J4 4, J5 5, J6 6, J7 7, J8 8
                )
            }

            pub fn fk_end_dyn(&self, q: &[f32]) -> RevampResult<Affine3A> {
                dynamic_fk!(@dispatch_fk_end self, q,
                    J1 1, J2 2, J3 3, J4 4, J5 5, J6 6, J7 7, J8 8
                )
            }
        }

        dynamic_fk!(@impl_fkchain $name, $chain, 1 J1, 2 J2, 3 J3, 4 J4, 5 J5, 6 J6, 7 J7, 8 J8);
    };

    (@dispatch_fk $self:ident, $q:ident, $($variant:ident $n:literal),+) => {
        match $self {
            $(Self::$variant(chain) => {
                let arr: &[f32; $n] = $q.try_into().map_err(|_| RevampError::ShapeMismatch {
                    expected: $n,
                    found: $q.len(),
                })?;
                Ok(FKChain::<$n>::fk(chain, &SRobotQ(*arr)).map_err(|e| -> RevampError { e.into() })?.to_vec())
            }),+
        }
    };

    (@dispatch_fk_end $self:ident, $q:ident, $($variant:ident $n:literal),+) => {
        match $self {
            $(Self::$variant(chain) => {
                let arr: &[f32; $n] = $q.try_into().map_err(|_| RevampError::ShapeMismatch {
                    expected: $n,
                    found: $q.len(),
                })?;
                FKChain::<$n>::fk_end(chain, &SRobotQ(*arr)).map_err(|e| -> RevampError { e.into() })
            }),+
        }
    };

    (@impl_fkchain $name:ident, $chain:ident, $($n:literal $variant:ident),+) => {
        $(
            impl FKChain<$n> for $name {
                type Error = RevampError;

                fn fk(&self, q: &SRobotQ<$n>) -> Result<[Affine3A; $n], Self::Error> {
                    match self {
                        Self::$variant(chain) => FKChain::<$n>::fk(chain, q).map_err(Into::into),
                        _ => Err(RevampError::ShapeMismatch {
                            expected: self.dof(),
                            found: $n,
                        }),
                    }
                }

                fn fk_end(&self, q: &SRobotQ<$n>) -> Result<Affine3A, Self::Error> {
                    match self {
                        Self::$variant(chain) => FKChain::<$n>::fk_end(chain, q).map_err(Into::into),
                        _ => Err(RevampError::ShapeMismatch {
                            expected: self.dof(),
                            found: $n,
                        }),
                    }
                }
            }

            impl From<$chain<$n>> for $name {
                fn from(chain: $chain<$n>) -> Self {
                    Self::$variant(chain)
                }
            }
        )+
    };
}

dynamic_fk!(DynamicDHChain, DHChain, DHJoint);
dynamic_fk!(DynamicHPChain, HPChain, HPJoint);
dynamic_fk!(DynamicURDFChain, URDFChain, URDFJoint);

impl DynamicDHChain {
    pub fn from_chain(chain: impl Into<Self>) -> Self { chain.into() }
}

impl DynamicHPChain {
    pub fn from_chain(chain: impl Into<Self>) -> Self { chain.into() }
}

impl DynamicURDFChain {
    pub fn from_chain(chain: impl Into<Self>) -> Self { chain.into() }
}

trait ErasedFK<const N: usize>: Send + Sync {
    fn fk(&self, q: &SRobotQ<N>) -> Result<[Affine3A; N], RevampError>;
    fn fk_end(&self, q: &SRobotQ<N>) -> Result<Affine3A, RevampError>;
    fn clone_box(&self) -> Box<dyn ErasedFK<N>>;
}

impl<const N: usize, FK: FKChain<N> + 'static> ErasedFK<N> for FK {
    fn fk(&self, q: &SRobotQ<N>) -> Result<[Affine3A; N], RevampError> {
        FKChain::fk(self, q).map_err(Into::into)
    }

    fn fk_end(&self, q: &SRobotQ<N>) -> Result<Affine3A, RevampError> {
        FKChain::fk_end(self, q).map_err(Into::into)
    }

    fn clone_box(&self) -> Box<dyn ErasedFK<N>> {
        Box::new(self.clone())
    }
}

pub struct BoxFK<const N: usize>(Box<dyn ErasedFK<N>>);

impl<const N: usize> BoxFK<N> {
    pub fn new(fk: impl FKChain<N> + 'static) -> Self {
        Self(Box::new(fk))
    }
}

impl<const N: usize> Clone for BoxFK<N> {
    fn clone(&self) -> Self {
        Self(self.0.clone_box())
    }
}

impl<const N: usize> FKChain<N> for BoxFK<N> {
    type Error = RevampError;

    fn fk(&self, q: &SRobotQ<N>) -> Result<[Affine3A; N], RevampError> {
        self.0.fk(q)
    }

    fn fk_end(&self, q: &SRobotQ<N>) -> Result<Affine3A, RevampError> {
        self.0.fk_end(q)
    }
}
