...
pub(crate) fn solve<const N: usize>(
    ...
    let coeffs = {
        let mut c = [0.0f64; N];
        for i in 0..N {
            c[i] = settings.rrtc.joint_coefficients.0[i] as f64;
        }
        c
    };
    ...
    let mut best_waypoints: Vec<SRobotQ<N>> = best_path
        ...
        .collect();
    let mut best_cost = path_cost(&best_waypoints, &coeffs);

    let c_min = weighted_distance(start, goal, &coeffs);
    ...
    let mut phs = Phs::new(start, goal, &coeffs);
    ...
    for _ in 0..settings.max_iterations {
        ...
        for inner in 0..settings.max_samples {
            ...
            let q_new = steer(&q_near, &q_rand, settings.rrtc.range, &coeffs);
            ...
            let mut new_cost = tree_a.cost(near_idx) + weighted_distance(&q_near, &q_new, &coeffs);

            if settings.cost_bound_resamples > 0 {
                for _ in 0..settings.cost_bound_resamples {
                    let near_nodes = tree_a.within(&q_new, new_cost - tree_a.cost(0));
                    let mut improved = false;
                    for &(cand_idx, cand_dist) in &near_nodes {
            ...
            let (connect_near, connect_dist) = tree_b.nearest(&q_new);
            ...
            loop {
                ...
                let dist = weighted_distance(&q_cur, &q_new, &coeffs);
                ...
                let q_step = steer(&q_cur, &q_new, settings.rrtc.range, &coeffs);

                if validate_edge(&q_cur, &q_step, settings.rrtc.resolution, validator).is_err() {
                    break;
                }

                let step_cost = tree_b.cost(cur_idx) + weighted_distance(&q_cur, &q_step, &coeffs);
            ...
            if connected {
                ...
                let actual_cost = path_cost(&waypoints, &coeffs);
        ...
        if !found_better || best_cost <= c_min + 1e-8 {
            break;
        }
    }
