...
use deke_types::{
    DekeError, DekeResult, FKChain, JointValidator, Retimer, SRobotPath, SRobotQ, SRobotTraj,
    Validator,
};

use crate::boundary::project;
use crate::constraints::{DensificationOptions, Topp3Tcp6DiscreteConstraints};
...
/// the strict verifier in [`crate::verify`] returns `Ok` to within the IPM
/// tolerance on every successful retime — no `resampled_check_slack` needed.
#[derive(Debug, Clone, Default)]
pub struct Topp3Tcp6Discrete;

impl<const N: usize> Retimer<N, f64> for Topp3Tcp6Discrete {
...
fn populate_path_geometry<const N: usize>(
    stats: &mut crate::diagnostic::PathStats,
...
fn topp_speed_seed<const N: usize, FK: FKChain<N, f64>>(
    deriv: &PathDerivatives<N>,
    path: &SRobotPath<N, f64>,
    constraints: &Topp3Tcp6DiscreteConstraints<N>,
    fk: &FK,
    ...
    let (result, _diag) = solver.retime(&spec, path, fk, &validator, &());
    let traj = result.ok()?;
...
// ── densification (duplicated from deke-topp3tcp6/retimer.rs) ────────────────
