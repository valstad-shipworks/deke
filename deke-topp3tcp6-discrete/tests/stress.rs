//! Stress tests aimed at finding failure modes in `Topp3Tcp6Discrete`.
//!
...
use deke_topp3tcp6_discrete::{
    BoundaryConditions, SolveStatus, TcpLimits, Topp3Tcp6Discrete, Topp3Tcp6DiscreteConstraints,
};
...
fn corner_180_reversal_single_joint() {
    ...
    let cfg = Topp3Tcp6DiscreteConstraints::<1>::symmetric(1.0, 2.0, 50.0);
    let mut validator = common::wide_validator::<1>();
    let (result, diag) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &mut validator, &());
...
fn zigzag_pattern() {
    ...
    let cfg = Topp3Tcp6DiscreteConstraints::<1>::symmetric(1.0, 5.0, 200.0);
    let mut validator = common::wide_validator::<1>();
    let (result, diag) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &mut validator, &());
...
fn wrist_only_rotation_with_tcp_bounds() {
    ...
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(2.0, 10.0, 500.0);
    cfg.tcp = Some(TcpLimits {
        v_max: 0.5,
        a_max: 5.0,
        j_max: 200.0,
...
fn all_limits_simultaneously_tight() {
    ...
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(0.8, 2.0, 30.0);
    cfg.tcp = Some(TcpLimits {
        v_max: 0.4,
        a_max: 2.0,
        j_max: 30.0,
...
fn long_path_many_waypoints() {
    ...
    let path = SRobotPath::<6, f64>::try_new(wps).unwrap();
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(1.5, 8.0, 400.0);
...
fn microscopic_path_length() {
    ...
    let cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(1.0, 5.0, 200.0);
    let mut validator = common::wide_validator::<6>();
    let (result, diag) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &mut validator, &());
...
fn high_velocity_boundary() {
    ...
    .unwrap();
    let mut cfg = Topp3Tcp6DiscreteConstraints::<1>::symmetric(1.0, 5.0, 200.0);
...
fn rail_dominant_with_tiny_arm_motion() {
    ...
    .unwrap();
    let mut cfg = Topp3Tcp6DiscreteConstraints::<7>::symmetric(1.0, 5.0, 200.0);
    cfg.tcp = Some(TcpLimits {
        v_max: 1.5,
        a_max: 10.0,
        j_max: 500.0,
...
fn asymmetric_joint_limits() {
    ...
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(1.0, 5.0, 200.0);
...
fn extremely_tight_tcp_jerk() {
    ...
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(2.0, 10.0, 1000.0);
    cfg.tcp = Some(TcpLimits {
        v_max: f64::INFINITY,
        a_max: f64::INFINITY,
...
fn single_joint_per_segment_rotation() {
    ...
    let path = SRobotPath::<6, f64>::try_new(wps).unwrap();
    let cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(1.5, 5.0, 200.0);
    let mut validator = common::wide_validator::<6>();
    let (result, diag) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &mut validator, &());
...
fn helical_tcp_path_with_tight_a() {
    ...
    let path = SRobotPath::<6, f64>::try_new(wps).unwrap();
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(2.0, 8.0, 400.0);
    cfg.tcp = Some(TcpLimits {
        v_max: 0.4,
        a_max: 1.0,
        j_max: 50.0,
...
fn zero_length_path_rejected_by_retimer() {
    ...
    let cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(1.5, 5.0, 200.0);
    let mut validator = common::wide_validator::<6>();
    let (result, _diag) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &mut validator, &());
    match result {
...
fn seven_dof_perfectly_correlated_dims() {
    ...
    let cfg = Topp3Tcp6DiscreteConstraints::<7>::symmetric(1.5, 5.0, 200.0);
    let mut validator = common::wide_validator::<7>();
    let (result, diag) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &mut validator, &());
...
fn pchip_spike_uneven_segments_4wp() {
    ...
    let path = SRobotPath::<6, f64>::try_new(wps).unwrap();
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(2.0, 8.0, 300.0);
...
fn mixed_per_joint_flip_versus_monotone() {
    ...
    let path = SRobotPath::<6, f64>::try_new(wps).unwrap();
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(1.5, 6.0, 250.0);
...
fn wrist_alignment_singularity_traverse() {
    ...
    let path = SRobotPath::<6, f64>::try_new(wps).unwrap();
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(1.5, 6.0, 200.0);
...
fn locked_base_curved_wrist() {
    ...
    let path = SRobotPath::<6, f64>::try_new(wps).unwrap();
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(2.0, 10.0, 400.0);
    cfg.locked_prefix = 3;
    let mut validator = common::wide_validator::<6>();
    let (result, diag) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &mut validator, &());
...
/// PCHIP machinery struggles with. The tolerance-relaxation retry in `Topp3Tcp6Discrete::retime`
...
/// here we assert the slack we promised in `DiscreteSolverOptions::boundary_slack`.
#[test]
