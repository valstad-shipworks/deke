//! Side-by-side comparison vs the existing continuous-time `deke-topp3tcp6`
//! retimer on a fixture fleet.
//!
//! Run with:
//!   cargo test -p deke-topp3tcp6-discrete --release --test side_by_side_bench \
//!     -- --ignored --nocapture
//!
//! Columns:
...
fn run_one<const N: usize, FK: deke_types::FKChain<N, f64>>(
    name: &str,
    ...
    let t = Instant::now();
    let (rc, dc) = Topp3Tcp6.retime(&cc, &path, fk, &validator, &());
    ...
    let t = Instant::now();
    let (rd, dd) = Topp3Tcp6Discrete.retime(&dc_cfg, &path, fk, &validator, &());
    ...
    let speed_solver = ToppSolver::<N, f64>::new(Duration::from_secs_f64(h));
    let t = Instant::now();
    let (rs, ds) = speed_solver.retime(&mspec, &path, fk, &validator, &());
