...
fn external_cfg() -> Topp3Tcp6DiscreteConstraints<6> {
    use deke_topp3tcp6_discrete::JointLimits;
...
fn bench_multi_seg_50wp_is_deterministic_within_binary() {
    ...
    let mut prev = None::<deke_topp3tcp6_discrete::Topp3Tcp6DiscreteDiagnostic>;
    ...
    for run in 0..3 {
        ...
        let (_result, diag) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &mut validator, &());
        eprintln!(
...
fn probe_2wp_long_chord_f32_fk() {
    ...
    let mut validator = wide_validator();
    let (result, diag) = Topp3Tcp6Discrete.retime(&external_cfg(), &path, &fk, &mut validator, &());
