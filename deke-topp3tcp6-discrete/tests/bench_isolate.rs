...
use deke_topp3tcp6_discrete::{TcpLimits, Topp3Tcp6Discrete, Topp3Tcp6DiscreteConstraints};
...
fn tcp_full() {
    ...
    cfg.tcp = Some(TcpLimits { v_max: 1.0, a_max: 5.0, j_max: 50.0 });
    let v = common::wide_validator::<6>();
    let t = std::time::Instant::now();
    let (r, d) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &v, &());
...
fn captured_10wp_no_seed() {
    ...
    cfg.tcp = Some(TcpLimits { v_max: 2.0, a_max: 20.0, j_max: 200.0 });
    cfg.solver.seed_from_topp_speed = false;
    let v = common::wide_validator::<6>();
    let t = std::time::Instant::now();
    let (r, d) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &v, &());
    eprintln!("captured_10wp_no_seed: {:?} in {:?}\n{}", r.is_ok(), t.elapsed(), d);
}
...
fn captured_10wp_with_seed() {
    ...
    let t = std::time::Instant::now();
    let (r, d) = Topp3Tcp6Discrete.retime(&cfg, &path, &fk, &v, &());
    eprintln!("captured_10wp_with_seed: {:?} in {:?}\n{}", r.is_ok(), t.elapsed(), d);
}
