...
fn external_chain() -> Kinematics<6, f64> {
    // 6 actuated revolute joints + 1 trailing fixed joint (folded into the tool
    // transform by `from_urdf`).
    let mut joints: Vec<URDFJoint> = URDF_JOINTS.to_vec();
    joints.extend_from_slice(&URDF_FIXED_SUFFIX);
    Kinematics::from_urdf(&joints, KinLimits::symmetric(f64::INFINITY), &[])
        .expect("URDF fixture is broken")
}
