...
/// plug straight into `Topp3Tcp6` (which demands an `FKChain<N, f64>`). This is
/// the replacement for the removed `FPDispatch` "author once, derive the
/// sibling precision" pattern.
#[test]
fn f32_built_chain_drives_f64_topp_retimer() {
    // Same 6-DOF DH geometry as `common::dh_6dof`, but built at f32 first.
    let dh = [
    ...
    // Sanity: end pose at zero is finite at both precisions.
