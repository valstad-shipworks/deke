...
fn external_chain() -> Kinematics<6, f64> {
    let mut joints: Vec<URDFJoint> = URDF_JOINTS.to_vec();
    joints.extend_from_slice(&URDF_FIXED_SUFFIX);
    match Kinematics::from_urdf(&joints, KinLimits::symmetric(f64::INFINITY), &[]) {
        Ok(c) => c,
        Err(e) => panic!(
            "URDF chain build failed on the external joints (fixture bug): {:?}",
            e
        ),
    }
}
...
fn external_cfg() -> Topp3Tcp6DiscreteConstraints<6> {
    use deke_topp3tcp_nlp::discrete::JointLimits;
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(1.0, 1.0, 1.0); // gets overwritten
...
/// Runs `Topp3Tcp6Discrete.retime` against the external URDF chain and dumps the diagnostic to
...
fn run_external_traj(
    ...
    cfg: Topp3Tcp6DiscreteConstraints<6>,
) -> bool {
...
fn smoke_external_chain_works() {
    ...
    assert!(
        ok,
        "external-chain smoke test failed; the fixture is broken"
    );
}
...
fn traj_template() {
    ...
    let mut cfg = Topp3Tcp6DiscreteConstraints::<6>::symmetric(1.5, 5.0, 200.0);
    cfg.tcp = Some(TcpLimits {
        v_max: f64::INFINITY,
        a_max: f64::INFINITY,
        j_max: f64::INFINITY,
...
fn bench_p2p_tiny_locally_infeasible() {
    let waypoints = vec![
        SRobotQ::from_array([
            1.0885119, 0.7386001, 0.4530678, 1.4205396, 0.1884099, 1.7257718,
        ]),
        SRobotQ::from_array([
            1.0519463, 0.7447735, 0.5023250, 1.4372716, 0.1531237, 1.7296510,
        ]),
    ];
...
fn bench_p2p_small_locally_infeasible() {
    let waypoints = vec![
        SRobotQ::from_array([
            -1.1703483, -0.3139854, 0.1235649, -1.0546926, 0.1807552, -0.9767784,
        ]),
        SRobotQ::from_array([
            -1.3687564, -0.4719839, 0.3012058, -1.1227008, 0.1640270, -0.9921301,
        ]),
    ];
...
fn bench_p2p_medium_locally_infeasible() {
    let waypoints = vec![
        SRobotQ::from_array([
            -0.6496428, -0.4632142, -0.6906568, 0.6976452, 0.1852536, 0.0864369,
        ]),
        SRobotQ::from_array([
            -0.9760553, -0.5811528, -0.3409180, 1.1595234, 0.4127398, 0.4162376,
        ]),
    ];
...
fn bench_multi_seg_50wp_is_deterministic_within_binary() {
    ...
    // captured trajectory at K≈3200 takes ~5 minutes wall in release. Two runs
    // already exercise the bit-for-bit-determinism property; a third catches any
    ...
    for run in 0..3 {
        ...
        eprintln!(
            ...
            run, diag.status, diag.iterations, diag.final_k, diag.output_fd_residual.joint_v,
        );
        if let Some(p) = &prev {
            assert_eq!(
            ...
            assert_eq!(diag.final_k, p.final_k, "final_k diverged",);
            ...
            assert_eq!(
                diag.output_fd_residual.joint_v.to_bits(),
                p.output_fd_residual.joint_v.to_bits(),
                "output_fd_residual.joint_v diverged",
            );
        }
        prev = Some(diag);
    }
...
fn bench_multi_seg_50wp_waypoints() -> Vec<SRobotQ<6, f64>> {
    vec![
        SRobotQ::from_array([
            0.3343222, 0.8929453, -0.0536905, 0.7182915, -1.2274445, -1.3873202,
        ]),
        SRobotQ::from_array([
            0.2674488, 0.9586826, -0.0377492, 0.6382554, -1.2414511, -1.3031950,
        ]),
        SRobotQ::from_array([
            0.3439960, 0.9346094, -0.1304580, 0.6939549, -1.2237945, -1.3037018,
        ]),
        SRobotQ::from_array([
            0.3770593, 0.8371893, -0.0471791, 0.7772376, -1.2808111, -1.3488145,
        ]),
        SRobotQ::from_array([
            0.3357331, 0.8777153, -0.0870164, 0.8393403, -1.2695598, -1.3989979,
        ]),
        SRobotQ::from_array([
            0.4185733, 0.9508877, -0.0808071, 0.9095306, -1.2537831, -1.3341339,
        ]),
        SRobotQ::from_array([
            0.4889850, 0.8706944, -0.0575931, 0.9365597, -1.3327671, -1.2880048,
        ]),
        SRobotQ::from_array([
            0.5089944, 0.7954211, -0.0015010, 0.9421552, -1.3384181, -1.1912302,
        ]),
        SRobotQ::from_array([
            0.5669821, 0.8019158, 0.0249302, 0.9983462, -1.2633414, -1.2682264,
        ]),
        SRobotQ::from_array([
            0.5462322, 0.8269755, 0.0114744, 1.0248954, -1.2638821, -1.1814715,
        ]),
        SRobotQ::from_array([
            0.4478502, 0.8240671, -0.0374530, 1.0970184, -1.2539433, -1.2531433,
        ]),
        SRobotQ::from_array([
            0.3921103, 0.8241266, 0.0245878, 1.1726546, -1.1683801, -1.3131421,
        ]),
        SRobotQ::from_array([
            0.2961473, 0.8801921, 0.0635315, 1.1099756, -1.1120828, -1.3019841,
        ]),
        SRobotQ::from_array([
            0.3483047, 0.8519227, 0.0110124, 1.0240691, -1.0440697, -1.2836198,
        ]),
        SRobotQ::from_array([
            0.2576151, 0.8472690, 0.0692917, 0.9619144, -1.0929707, -1.3585659,
        ]),
        SRobotQ::from_array([
            0.2756267, 0.8216234, 0.0963444, 0.9842113, -1.1360959, -1.3027613,
        ]),
        SRobotQ::from_array([
            0.3094045, 0.8512013, 0.1763514, 1.0179800, -1.0990852, -1.3464782,
        ]),
        SRobotQ::from_array([
            0.2801693, 0.9165845, 0.2148244, 0.9510065, -1.0831171, -1.2620443,
        ]),
        SRobotQ::from_array([
            0.2177491, 0.9850859, 0.2158370, 0.8781699, -1.1183024, -1.2107089,
        ]),
        SRobotQ::from_array([
            0.2758391, 0.8904893, 0.1827938, 0.9697054, -1.0520904, -1.2573381,
        ]),
        SRobotQ::from_array([
            0.2849699, 0.7979505, 0.1046686, 0.9483294, -1.0480146, -1.3293309,
        ]),
        SRobotQ::from_array([
            0.1961584, 0.8614633, 0.1554078, 0.9999405, -1.1274721, -1.3711356,
        ]),
        SRobotQ::from_array([
            0.2676405, 0.7901247, 0.0935843, 1.0396187, -1.1730380, -1.3732298,
        ]),
        SRobotQ::from_array([
            0.1936202, 0.6951341, 0.0108883, 1.1101067, -1.2299142, -1.4591768,
        ]),
        SRobotQ::from_array([
            0.2042345, 0.7925586, -0.0770198, 1.1643602, -1.1788505, -1.3601731,
        ]),
        SRobotQ::from_array([
            0.1772767, 0.7440868, -0.0045473, 1.2396978, -1.2647126, -1.3096728,
        ]),
        SRobotQ::from_array([
            0.2500148, 0.7903014, 0.0535562, 1.1545478, -1.2553731, -1.3982640,
        ]),
        SRobotQ::from_array([
            0.3239738, 0.6914120, 0.1236605, 1.1396301, -1.3122255, -1.4305973,
        ]),
        SRobotQ::from_array([
            0.3457220, 0.7121638, 0.1943251, 1.0607872, -1.2247261, -1.4396483,
        ]),
        SRobotQ::from_array([
            0.2652950, 0.6738650, 0.2469884, 1.0853137, -1.3219487, -1.4402719,
        ]),
        SRobotQ::from_array([
            0.2848045, 0.7311544, 0.3184022, 1.1667714, -1.3546409, -1.3477301,
        ]),
        SRobotQ::from_array([
            0.2737518, 0.7860500, 0.4029508, 1.2577620, -1.3499184, -1.3142234,
        ]),
        SRobotQ::from_array([
            0.2533161, 0.8850348, 0.4109326, 1.2623707, -1.4458526, -1.3847241,
        ]),
        SRobotQ::from_array([
            0.2767170, 0.9426061, 0.4777802, 1.2599477, -1.4371838, -1.3086173,
        ]),
        SRobotQ::from_array([
            0.2284535, 0.9425596, 0.4273691, 1.2964639, -1.4957550, -1.2169469,
        ]),
        SRobotQ::from_array([
            0.1689166, 0.9606519, 0.3400774, 1.2576398, -1.5093432, -1.2456689,
        ]),
        SRobotQ::from_array([
            0.2607760, 0.8947233, 0.3993521, 1.2092380, -1.5728505, -1.1910087,
        ]),
        SRobotQ::from_array([
            0.2070883, 0.8881449, 0.4243983, 1.1515865, -1.6702937, -1.2591626,
        ]),
        SRobotQ::from_array([
            0.1682711, 0.8673760, 0.3575768, 1.1450649, -1.7561177, -1.2556389,
        ]),
        SRobotQ::from_array([
            0.0752571, 0.8218140, 0.4143286, 1.1751732, -1.8027944, -1.1714458,
        ]),
        SRobotQ::from_array([
            0.0273413, 0.9066567, 0.3907296, 1.1128650, -1.7628050, -1.1570527,
        ]),
        SRobotQ::from_array([
            0.0327829, 0.8962876, 0.3926617, 1.1951643, -1.6699687, -1.0671434,
        ]),
        SRobotQ::from_array([
            0.0685963, 0.9272223, 0.4217075, 1.2061300, -1.7079343, -0.9955891,
        ]),
        SRobotQ::from_array([
            0.1085894, 0.8989490, 0.4181724, 1.2784317, -1.7519532, -1.0057845,
        ]),
        SRobotQ::from_array([
            0.0601945, 0.8779040, 0.3456943, 1.2673952, -1.8081218, -0.9385045,
        ]),
        SRobotQ::from_array([
            0.0829960, 0.8381892, 0.3949460, 1.2175243, -1.7146095, -1.0313196,
        ]),
        SRobotQ::from_array([
            0.1520397, 0.7552742, 0.4169602, 1.1308244, -1.7895352, -1.0720215,
        ]),
        SRobotQ::from_array([
            0.1429673, 0.6877099, 0.4091094, 1.1654123, -1.8709186, -1.1462775,
        ]),
        SRobotQ::from_array([
            0.2112407, 0.6016545, 0.3173946, 1.1989725, -1.8561647, -1.0833602,
        ]),
        SRobotQ::from_array([
            0.1451975, 0.6494479, 0.2705453, 1.1745205, -1.8705170, -1.1463838,
        ]),
    ]
}
...
fn probe_2wp_long_chord_b_single_stage() {
    let waypoints = vec![
        ...
        SRobotQ::from_array([
            ...
            -0.6136122566303709,
            -0.30540762557050655,
            0.012720444323652685,
        ]),
    ];
    let cfg = external_cfg();
    // two_stage_warm_start removed in discrete crate
...
fn probe_2wp_long_chord_f32_fk() {
    // Build the chain at f32 precision, then read its structure back into an
    // f64 chain — mirrors how production constructs FK from an f32 source.
    let mut joints: Vec<URDFJoint> = URDF_JOINTS.to_vec();
    joints.extend_from_slice(&URDF_FIXED_SUFFIX);
    let fk_f32 =
        Kinematics::<6, f32>::from_urdf(&joints, KinLimits::symmetric(f32::INFINITY), &[]).unwrap();
    let fk: Kinematics<6, f64> = fk_f32.to_f64();
    let waypoints = vec![
        SRobotQ::from_array([
            -0.8204609515172723,
            0.31613102569329266,
    ...
    let validator = wide_validator();
    let (result, diag) =
        Topp3Tcp6Discrete::new(&fk).retime(&external_cfg(), &path, &validator, &());
...
fn tight_arm_cfg() -> Topp3Tcp6DiscreteConstraints<6> {
    use deke_topp3tcp_nlp::discrete::JointLimits;
    let mut cfg = external_cfg();
    cfg.joint = JointLimits {
        q_min: SRobotQ::from_array([f64::NEG_INFINITY; 6]),
        ...
        j_max: SRobotQ::from_array([
            16.456684, 14.105729, 13.322078, 18.807639, 18.807639, 31.346066,
        ]),
    };
    cfg
}
...
fn external_2wp_tight_chord_3p27() {
    let waypoints = vec![
        ...
        SRobotQ::from_array([
            0.002662742298985527,
            0.14094291649646548,
            -0.15317010814094578,
            0.0010269920539685462,
            0.15082150296467675,
            0.003009367616041299,
        ]),
    ];
...
fn external_2wp_tight_chord_1p96() {
    let waypoints = vec![
        ...
        SRobotQ::from_array([
            0.0026627422990204686,
            0.14094291649646626,
            -0.15317010814094092,
            0.0010269920533265077,
            0.15082150296466543,
            0.0030093676166768456,
        ]),
    ];
...
fn external_2wp_long_chord_slack_edge() {
    ...
    let ok = run_external_traj(
        "external_2wp_long_chord_slack_edge",
        waypoints,
        external_cfg(),
    );
    assert!(ok, "still failing — check diagnostic above");
...
fn external_bench_multi_seg_10wp_run3_single_stage() {
    ...
    assert!(
        ok,
        "single-stage also failing — path may need different fix"
    );
}
