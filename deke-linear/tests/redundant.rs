...
use deke_linear::{
    FollowConfig, JointLimits, LinearFollower, RedundantAxis, RedundantOptions,
};
...
fn redundant_follow_resolves_yaw_and_runs() {
    ...
    let cfg = FollowConfig::weld(35.0, JointLimits::symmetric(2.0, 8.0, 80.0), Duration::from_millis(8))
    ...
    let (traj, diag) = follower.follow(&poses, &cfg, &common::noop(), &()).expect("redundant follow failed");
    ...
    assert!(lo >= -45f64.to_radians() - 1e-6 && hi <= 45f64.to_radians() + 1e-6,
        "resolved yaw must stay inside the window: [{lo}, {hi}]");
    assert!(rd.min_manipulability > 0.0, "track should stay off singularities");
    ...
    for &v in &speeds[lo_i..hi_i] {
        assert!((v - expected).abs() < expected * 0.15, "cruise {v} vs {expected}");
    }
}
...
fn axis_choice_changes_the_solution() {
    ...
    let base = |axis| {
        FollowConfig::weld(30.0, joint.clone(), Duration::from_millis(8)).with_redundancy(
            RedundantOptions { axis, yaw_samples: 16, ..RedundantOptions::default() },
        )
    };

    let (tz, _) = follower.follow(&poses, &base(RedundantAxis::PosZ), &common::noop(), &()).unwrap();
    let (tx, _) = follower.follow(&poses, &base(RedundantAxis::PosX), &common::noop(), &()).unwrap();
    ...
    assert!(max_diff > 1e-3, "freeing X vs Z should change the joint solution");
}
